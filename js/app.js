(function() {
	...
	var canvasPainting = $("#canvas-painting")[0];

	var contextSource = canvasSource.getContext('2d');
	var contextBlended = canvasBlended.getContext('2d');
	var contextPainting = canvasPainting.getContext('2d');
	...
	function drawPainting() {
		...
		if (!paintingData) {
			paintingData = contextPainting.createImageData(canvasPainting.width, canvasPainting.height);
		}

		var blendedData = contextBlended.getImageData(0, 0, canvasBlended.width, canvasBlended.height);
		for (var i = 0; i < blendedData.data.length * 0.25; i++) {
			if (blendedData.data[i*4] != 0) {
				paintingData.data[i*4] = selectedColor.r;
				paintingData.data[i*4+1] = selectedColor.g;
				paintingData.data[i*4+2] = selectedColor.b;
			}
			paintingData.data[i*4+3] = 0xFF;
		}

		contextPainting.putImageData(paintingData, 0, 0);
	}
